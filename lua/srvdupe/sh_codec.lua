-- From https://github.com/wiremod/advdupe2/blob/master/lua/advdupe2/sh_codec.lua
...
local REVISION = 5
SrvDupe.CodecRevision = REVISION
SrvDupe.MaxDupeSize = 32e6 -- 32 MB
...
function SrvDupe.Encode(dupe, info, callback, ...)
...
versions[1] = SrvDupe.LegacyDecoders[1]
versions[2] = SrvDupe.LegacyDecoders[2]
...
versions[4] = function(encodedDupe)
    ...
    return deserialize(decompress(dupestring, SrvDupe.MaxDupeSize), read4), info
end

versions[5] = function(encodedDupe)
    local info, dupestring = getInfo(encodedDupe:sub(7))
    return deserialize(decompress(dupestring, SrvDupe.MaxDupeSize), read5), info
end

function SrvDupe.CheckValidDupe(dupe, info)
...
function SrvDupe.Decode(encodedDupe)
    ...
    if sig ~= "AD2F" then
        if sig == "[Inf" then --legacy support, ENGAGE (AD1 dupe detected)
            local success, tbl, info, moreinfo = pcall(SrvDupe.LegacyDecoders[0], encodedDupe)
            ...
            if success then
                success, tbl = SrvDupe.CheckValidDupe(tbl, info)
            end
